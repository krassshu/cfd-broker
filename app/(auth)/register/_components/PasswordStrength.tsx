        ...
        <div className="space-y-3 !mt-4">
            ...
            <div className="grid grid-cols-2 gap-2">
                {requirements.map((req, i) => (
                    <div key={i} className={`text-[10px] flex items-center gap-1 ${req.re.test(password) ? 'text-emerald-500' : 'text-muted/50'}`}>
                        <div className={`w-1 h-1 rounded-full ${req.re.test(password) ? 'bg-emerald-500' : 'bg-muted/50'}`} />
                        {req.label}
                    </div>
                ))}
            </div>
